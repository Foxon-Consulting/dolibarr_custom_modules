...
?>
	...
	if (!empty($extrafields)) {
		...
		if (!empty($temps)) {
			print '<div style="padding-top: 10px" id="extrafield_lines_area_edit" name="extrafield_lines_area_edit" class="yolo">';
			print $temps;
			print '</div>';
		}
	}
    ...
    <td class="right">
    <?php
    if (in_array($object->element, array('propal', 'facture')))
    {
        $coldisplay++;
        print '<input size="3" type="text" class="flat right" name="options_days" id="options_days" value="'.(GETPOSTISSET('options_days') ? GETPOST('options_days') : $line->array_options['options_days'] ?? 1).'"';
    }
    ?>
    </td>
