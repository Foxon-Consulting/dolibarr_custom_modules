...
if ($nolinesbefore) {
	...
	<tr class="liste_titre<?php echo (($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_add_') ?> nodrag nodrop">
      ...
      <td class="linecoldays right"><?php echo $langs->trans('Days'); ?></td>
		<?php
...
<tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
    ...
    ?>
        <td class="nobottom linecolqty right">
        <?php $default_days = 1; ?>
        <input type="text" name="options_days" id="options_days" class="flat width40 right" value="<?php echo (GETPOSTISSET("options_days") ? GETPOST("options_days", 'alpha', 2) : $default_days); ?>">
        </td>
    <?php
    }
    ?>
